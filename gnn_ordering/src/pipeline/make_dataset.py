...
def main():
    ...
    # ---- fixed split: create once (round0), reuse forever ----
    ...
    np.savez(os.path.join(args.out_dir, "train.npz"), X=Xtr, y=ytr, graph_id=np.array(gid_tr, dtype=object))
    np.savez(os.path.join(args.out_dir, "val.npz"), X=Xva, y=yva, graph_id=np.array(gid_va, dtype=object))
    np.savez(os.path.join(args.out_dir, "test.npz"), X=Xte, y=yte, graph_id=np.array(gid_te, dtype=object))
