...
def modified_kendall_tau_distance(order_a: np.ndarray, order_b: np.ndarray, type=1) -> int:
    """
    Calculates a weighted Kendall tau distance between two permutations.

        Apply a weight based on positions in order_a so that discordant pairs are not all counted equally.
        Items in order_a are grouped into buckets that grow exponentially with position.
        The weight of pair (i, j) is 0.5^(bucket_i + bucket_j), so that items appearing earlier in order_a
    ...
    for i, node in enumerate(order_b):
        pos_b[int(node)] = i
    temp = [] # [0, 1, 1, 2, 2, 2, 2, 3, ...]
    k = 0
    while len(temp) < n:
        temp.extend([k] * (1 << k))  # 2^k copies of k
        k += 1
    bucket = [0] * n
    for i in range(n):
        bucket[order_a[i]] = temp[i]
    dist = 0
    for i in range(n):
        for j in range(i + 1, n):
            dist += int((pos_a[i] < pos_a[j]) != (pos_b[i] < pos_b[j])) * (0.5 ** (bucket[i] + bucket[j]))
            # a couple alternative schemes:
            # dist += int((pos_a[i] < pos_a[j]) != (pos_b[i] < pos_b[j])) * (0.9 ** (n + n - pos_a[i] - pos_a[j]))
            # if (bucket[i] != bucket[j]):
            #     dist += int((pos_a[i] < pos_a[j]) != (pos_b[i] < pos_b[j]))
    return dist


def modified_kendall_tau_similarity_from_distance(K: int, n: int) -> float:
    """Modified Kendall tau similarity in [-1,1]:"""
    bucket = [] # [0, 1, 1, 2, 2, 2, 2, 3, ...]
    k = 0
    while len(bucket) < n:
        bucket.extend([k] * (1 << k))  # 2^k copies of k
        k += 1
    kmax = 0
    for i in range(n):
        for j in range(i + 1, n):
            kmax += 0.5 ** (bucket[i] + bucket[j])
    if kmax == 0:
        return 1.0
    return float(1.0 - 2.0 * (K / kmax))
...
def diagnostic_score_vs_kendall(
    ...
    for _ in range(n_random):
        ...
        K = modified_kendall_tau_distance(true_order, order)
        kt = modified_kendall_tau_similarity_from_distance(K, n)
...
def main(
    ...
    for idx, inst in enumerate(test_insts):
        ...
        K = modified_kendall_tau_distance(true_order, pred_order)
        ktau = modified_kendall_tau_similarity_from_distance(K, n_nodes)
